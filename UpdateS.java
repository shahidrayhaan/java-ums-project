...
public class UpdateS extends JFrame implements ActionListener{
    ...
    UpdateS(){
        ...
        add(heading);
        JLabel roll = new JLabel("Select Registration Number");
        roll.setBounds(5,70,200,30);
        add(roll);
        sroll = new Choice();
        sroll.setBounds(215,70,150,30);
        add(sroll);
        try{
            Conn c = new Conn();
            ResultSet rs=c.s.executeQuery("select * from student");
            while(rs.next()){
                sroll.add(rs.getString("reg"));
            }
        }catch(Exception e){
            e.printStackTrace();
        }
        ...
        add(ffname);
        JLabel rollno = new JLabel("Reg no");
        ...
        try{
            Conn c = new Conn();
            String query = "select * from student where reg='"+sroll.getSelectedItem()+"'";
            ResultSet rs =c.s.executeQuery(query);
            while(rs.next()){
                labelname.setText(rs.getString("name"));
                ffname.setText(rs.getString("fname"));
                labelroll.setText(rs.getString("reg"));
                ...
                bbranch.setText(rs.getString("branch"));
            ...
            if(ae.getSource()==submit){
              String aaddress =  address.getText();
               ...
               try{
                         ...
                         String query= "update  student set address='"+aaddress+"',phone='"+phone+"',email='"+emailid+"',course='"+course+"',branch='"+branch+"' where reg='"+roll+"'";
                          Conn c = new Conn();
